...
```bash
curl -X POST http://192.168.1.23:8001/api/switch \
...
```bash
curl http://192.168.1.23:8001/api/status
```
...
```bash
curl http://192.168.1.23:8001/api/roles
```
...
The API reads role→model mappings from `data/agent-roles.json` and uses `~/llm-switch.sh` to swap models on the Jetson. Only one model runs at a time (8GB constraint).

## Requirements

- Python 3.8+ (stdlib only)
- `~/llm-switch.sh` configured on the Jetson
...
3. Add model entry to `~/llm-switch.sh` on the Jetson
