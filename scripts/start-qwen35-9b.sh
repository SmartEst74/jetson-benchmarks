...
MODEL_PATH="${MODEL_PATH:-/home/jetson/models/llama-cache/Qwen3.5-9B-Q4_K_M.gguf}"
...
exec sudo docker run --rm --name qwen35-9b \
  ...
  "$IMAGE" \
  llama-server \
    $(if [[ -f "$MODEL_PATH" ]]; then echo "--model /models/$MODEL_BASENAME"; else echo "--hf-repo unsloth/Qwen3.5-9B-GGUF --hf-file Qwen3.5-9B-Q4_K_M.gguf"; fi) \
    --ctx-size "$CTX" \
    --n-gpu-layers 999 \
    --flash-attn on \
    --parallel 1 \
